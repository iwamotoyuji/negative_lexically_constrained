...
bpe_path=${DATASETS_DIR}/newsela-auto/bpe${BPE_TOKENS}
mkdir -p $bpe_path

if [ $BPE_TOKENS == "BART" ]; then
    for file in train.complex train.simple valid.complex valid.simple test.complex; do
        python ${PRE_TRAINED_DIR}/multiprocessing_bpe_encoder.py \
    ...
    for file in valid.complex valid.simple test.complex; do
