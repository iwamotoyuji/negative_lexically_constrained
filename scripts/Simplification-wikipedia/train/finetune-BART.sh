...
while getopts ab:d:g:hln:p:r:s: OPT
do
    case $OPT in
    ...
    g ) GPU_ID=$OPTARG
        ;;
    h ) echo "Usage: $0 [-a TRAIN_SET=wiki-auto] [-b BPE_TOKENS] [-d DATASETS_DIR] [-g GPU_ID] [-l BART_SCALE=large] [-n EXP_NAME] [-p PRE_TRAINED_DIR] [-r RESULT_DIR] [-s SEED]" 1>&2
        exit 1
        ;;
    l ) BART_SCALE=large
        ;;
...
save_dir=${RESULT_DIR}/${TRAIN_SET}/${EXP_NAME}
rm -fr $save_dir
mkdir -p $save_dir
